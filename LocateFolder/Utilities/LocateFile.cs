...
namespace LocateFolder.Utilities
{
    internal static class LocateFile
    {
        ...
        public static void FileOrFolder(string path, bool edit = false)
        {
            ...
            var pidlFolder = PathToAbsolutePIDL(path);
        ...
        public static void FilesOrFolders(string parentDirectory, ICollection<string> filenames)
        {
            ...
            {
                var pidl = PathToAbsolutePIDL(parentDirectory);

                try
                {
                    var parentFolder = PIDLToShellFolder(pidl);
                    var list = new List<IntPtr>(filenames.Count);
        ...
        private class NativeMethods
        {
            ...
            [DllImport("ole32.dll", EntryPoint = nameof(CreateBindCtx))]
            ...
            [DllImport("shell32.dll", EntryPoint = nameof(SHGetDesktopFolder), CharSet = CharSet.Unicode, SetLastError = true)
            ]
            ...
            {
                var cidl = (apidl != null) ? ((uint)apidl.Length) : 0;
                Marshal.ThrowExceptionForHR(SHOpenFolderAndSelectItems_(pidlFolder, cidl, apidl, dwFlags));
            }

            [DllImport("shell32.dll", EntryPoint = nameof(SHOpenFolderAndSelectItems))]
